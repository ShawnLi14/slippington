...
## Directional launch from an angled launcher pad (owning peer only). Vertical-
## dominant: it sets both components as an impulse, but the sideways throw is NOT
## held — normal air control resumes next frame, so you steer the arc yourself.
## (MapPlanner._launcher_edge_ok models exactly this: vertical apex from vel.y,
## horizontal reach by air control from the pad, not by crediting vel.x as drift.)
